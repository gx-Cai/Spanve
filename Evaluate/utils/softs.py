...
class SpatialSoft():
    ...
    def load_func(self):
        if self.name == 'spanve-k':
            ...
            return run_meringue
        elif self.name == 'seurat':
            return lambda data,saving_path: sc_methods(data,saving_path,flavor='seurat')
        elif self.name == 'cell_ranger':
            return lambda data,saving_path: sc_methods(data,saving_path,flavor='cell_ranger')
        elif self.name == 'seurat_v3':
            return lambda data,saving_path: sc_methods(data,saving_path,flavor='seurat_v3')
        else:
...
def sc_methods(adata, saving_path, flavor, **kwargs):
    import scanpy as sc
    ...
    if flavor in ['seurat', 'cell_ranger']:
        ...
        adata.var.to_csv(os.path.join(saving_path,f'sc-{flavor}.csv'))
    elif flavor == 'seurat_v3':
        sc.pp.highly_variable_genes(adata, flavor = 'seurat_v3', n_top_genes = adata.shape[1]//10, span=0.5)
        adata.var.fillna('NA').to_csv(os.path.join(saving_path,f'sc-{flavor}.csv'))
