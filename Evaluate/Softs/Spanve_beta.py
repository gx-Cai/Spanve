...
__version__ = '0.1.4_beta_2024.02.27'
...
def persent_select(arr,prop=0.95):
...
def modularity(X, labels):
    """
    X: (n_sample, n_sample) affinity matrix of the network
    labels: (n_sample, ) the labels of the nodes
    """
...
class Spanve(object):
        ...
        **kwargs
        ...
        if clustering_method == 'sklearn':
            ...
            if n_clusters is None or isinstance(n_clusters, list):
                if n_clusters is None: n_clusters = [3, 10]
    ...
    def fit(self, verbose=None, force_reject = False):
        ...
        if self.rejects.sum() < 1 and force_reject:
            ...
            -----------------------------------------------
            Adjusted `rejetcs` by top 5 persent genes. You can still see fdrs in attribute `self.fdrs`.
    ...
    def load_df(self, df, verbose=True):
        self.result_df = df
